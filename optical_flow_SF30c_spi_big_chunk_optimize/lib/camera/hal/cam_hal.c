...
void IRAM_ATTR ll_cam_send_event(cam_obj_t *cam, cam_event_t cam_event, BaseType_t * HPTaskAwoken)
{
    if (xQueueSendFromISR(cam->event_queue, (void *)&cam_event, HPTaskAwoken) != pdTRUE) {
        ...
        CAM_WARN_THROTTLE(ovf_cnt,cam_event==CAM_IN_SUC_EOF_EVENT ? "EV-EOF-OVF" : "EV-VSYNC-OVF");
#endif
    }
}
                    ...
                    if (cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode) {
                        ...
                        if (cam_obj->psram_mode) {
                            ...
                            if (frame_buffer_event->len != cam_obj->fb_size) {
                                static uint16_t fb_size_miss_cnt = 0;
                                cam_obj->frames[frame_pos].en = 1;
                                CAM_WARN_THROTTLE(fb_size_miss_cnt, "FB-SIZE-MISMATCH");
                            }
                        }
                        //send frame
...
static ovcam_err_t cam_dma_config(const camera_config_t *config)
{
    ...
    // cam_obj->frame_copy_cnt =(cam_obj->recv_size + cam_obj->dma_half_buffer_size - 1)/ cam_obj->dma_half_buffer_size; 
    // cam_obj->frame_copy_cnt = (cam_obj->recv_size + cam_obj->dma_node_buffer_size - 1) / cam_obj->dma_node_buffer_size;
    ...
    //          (int) cam_obj->dma_node_cnt, (int) cam_obj->frame_copy_cnt);
        ...
        size_t in_node_bytes  = cam_obj->dma_node_buffer_size;
        size_t out_per_node   = (in_node_bytes * cam_obj->fb_bytes_per_pixel)
                            / (cam_obj->dma_bytes_per_item * cam_obj->in_bytes_per_pixel);

        // *** non-JPEG ***: คาดจำนวน EOF = เพดาน(fb_size / out_per_node)
        if (!cam_obj->jpeg_mode) {
            size_t expected = cam_obj->fb_size;
            cam_obj->frame_copy_cnt = (expected + out_per_node - 1) / out_per_node;   // เช่น 153600/480 = 320
        } else {
            // *** JPEG ***: ความยาวแปรผัน ใช้เพดานเท่านั้น (ไม่ใช้เป็นเกณฑ์หยุดจริง)
            size_t cap = cam_obj->fb_size;
            cam_obj->frame_copy_cnt = (cap + out_per_node - 1) / out_per_node;        // soft cap เฉย ๆ
        }
    SCCB_LOGI(TAG, "dma_buffer_size: %d, dma_half_buffer_size: %d, dma_node_buffer_size: %d, dma_node_cnt: %d, frame_copy_cnt: %d",
        (int) cam_obj->dma_buffer_size, (int) cam_obj->dma_half_buffer_size, (int) cam_obj->dma_node_buffer_size,
        (int) cam_obj->dma_node_cnt, (int) cam_obj->frame_copy_cnt);
    ...
    SCCB_LOGD(TAG, "frames=%p, cnt=%u, each=%zu, total=%zu",(void*)cam_obj->frames,(unsigned)cam_obj->frame_cnt,sizeof(cam_frame_t),(size_t)cam_obj->frame_cnt * sizeof(cam_frame_t));

    CAM_CHECK(cam_obj->frames != NULL, "frames malloc failed", OVCAM_E_FAIL);
    ...
    for (int x = 0; x < cam_obj->frame_cnt; x++) {
        ...
        // ESP_LOGI(TAG, "Allocating %d Byte frame buffer in %s", alloc_size, _caps & MALLOC_CAP_SPIRAM ? "PSRAM" : "OnBoard RAM");
         SCCB_LOGI(TAG, "Allocating %d Byte frame buffer in %s", alloc_size, _caps & MALLOC_CAP_SPIRAM ? "PSRAM" : "OnBoard RAM");
        ...
        if (cam_obj->psram_mode) {
            ...
            SCCB_LOGI(TAG, "Frame[%d]: Offset: %u, Addr: 0x%08X", x, cam_obj->frames[x].fb_offset, (unsigned) cam_obj->frames[x].fb.buf);
    ...
    if (!cam_obj->psram_mode) {
        ...
        uintptr_t start = (uintptr_t)cam_obj->dma_buffer;
        uintptr_t end   = start + cam_obj->dma_buffer_size - 1;

        SCCB_LOGD(TAG, "dma_buffer=%p size=%u  range: 0x%08X - 0x%08X", (void*)cam_obj->dma_buffer,(unsigned)cam_obj->dma_buffer_size,(unsigned)start, (unsigned)end);
        ...
        if(NULL == cam_obj->dma_buffer) {
            // ESP_LOGE(TAG,"%s(%d): DMA buffer %d Byte malloc failed, the current largest free block:%d Byte", __FUNCTION__, __LINE__,
                    ...
                    SCCB_LOGE(TAG,"%s(%d): DMA buffer %d Byte malloc failed, the current largest free block:%d Byte", __FUNCTION__, __LINE__,
                     (int) cam_obj->dma_buffer_size, (int) heap_caps_get_largest_free_block(MALLOC_CAP_DMA));
            return OVCAM_E_FAIL;
        }
...
ovcam_err_t cam_init(const camera_config_t *config)
{
    ...
    CAM_CHECK_GOTO(ret == OVCAM_OK, "ll_cam initialize failed", err);
...
ovcam_err_t cam_config(const camera_config_t *config, framesize_t frame_size, uint16_t sensor_pid)
{
    CAM_CHECK(NULL != config, "config pointer is invalid", ESP_ERR_INVALID_ARG);
    ovcam_err_t ret = OVCAM_OK;

    ret = ll_cam_set_sample_mode(cam_obj, (pixformat_t)config->pixel_format, config->xclk_freq_hz, sensor_pid);
    CAM_CHECK_GOTO(ret == OVCAM_OK, "ll_cam_set_sample_mode failed", err);
    ...
    SCCB_LOGI(TAG, "PSRAM DMA mode %s", cam_obj->psram_mode ? "enabled" : "disabled");
    SCCB_LOGD(TAG,"psram status -- %d",(int)cam_obj->psram_mode);
    ...
    CAM_CHECK_GOTO(ret == OVCAM_OK, "cam_dma_config failed", err);

    size_t queue_size = cam_obj->dma_half_buffer_cnt ;
    if (queue_size == 0) {
        queue_size = 1;
    }
    SCCB_LOGD(TAG,"dma_half_buffer_cnt : %u",(unsigned)queue_size);
    ...
    CAM_CHECK_GOTO(ret == OVCAM_OK, "cam intr alloc failed", err);
    ...
    SCCB_LOGI(TAG,
        "cfg: jpeg=%d psram=%d W=%u H=%u in_bytes_per_pixel=%u cam_obj->fb_bytes_per_pixel=%u cam_obj->recv_size=%u cam_obj->fb_size=%u cam_obj->dma_half_buffer_size=%u cam_obj->dma_half_buffer_cnt=%u cam_obj->frame_copy_cnt=%u expected_cnt=%u",
        cam_obj->jpeg_mode, cam_obj->psram_mode,
        cam_obj->width, cam_obj->height,
        cam_obj->in_bytes_per_pixel, cam_obj->fb_bytes_per_pixel,
        (unsigned)cam_obj->recv_size, (unsigned)cam_obj->fb_size,
        (unsigned)cam_obj->dma_half_buffer_size,
        (unsigned)cam_obj->dma_half_buffer_cnt,
        (unsigned)cam_obj->frame_copy_cnt,
        (unsigned)((cam_obj->recv_size + cam_obj->dma_half_buffer_size - 1) / cam_obj->dma_half_buffer_size) // expected
    );
    // ESP_LOGI(TAG, "cam config ok");
    SCCB_LOGI(TAG, "cam config ok");
    return OVCAM_OK;
...
camera_fb_t *cam_take(TickType_t timeout)
{
    ...
    for (;;)
    {
        ...
        if (elapsed >= timeout) {
            ...
            SCCB_LOGW(TAG, "Failed to get frame: timeout");
            return NULL;
        }
