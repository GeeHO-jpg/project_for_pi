...
typedef struct {
    uint8_t  *buf;         // ชี้ไปที่ dma_buffer เดิม
    size_t    node_size;   // dma_node_buffer_size (ไบต์ขาเข้า/ต่อ 1 node)
    uint32_t  node_cnt;    // dma_node_cnt (จำนวน node ทั้งหมดในบัฟเฟอร์วงแหวน)
} DmaRing;
