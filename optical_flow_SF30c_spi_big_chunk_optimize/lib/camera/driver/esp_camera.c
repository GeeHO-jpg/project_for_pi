...
#include "../camera/hal/cam_hal.h"
#include "../camera/driver/esp_camera.h"
#include "../camera/hal/xclk.h"
#if CONFIG_OV2640_SUPPORT
#include "../sensor/ov2640.h"
#endif
#if CONFIG_OV7725_SUPPORT
#include "../sensor/ov7725.h"
#endif
#if CONFIG_OV3660_SUPPORT
#include "../sensor/ov3660.h"
#endif
#if CONFIG_OV5640_SUPPORT
#include "../sensor/ov5640.h"
#endif
#if CONFIG_NT99141_SUPPORT
#include "../sensor/nt99141.h"
#endif
#if CONFIG_OV7670_SUPPORT
#include "../sensor/ov7670.h"
#endif
#if CONFIG_GC2145_SUPPORT
#include "../sensor/gc2145.h"
#endif
#if CONFIG_GC032A_SUPPORT
#include "../sensor/gc032a.h"
#endif
#if CONFIG_GC0308_SUPPORT
#include "../sensor/gc0308.h"
#endif
#if CONFIG_BF3005_SUPPORT
#include "../sensor/bf3005.h"
#endif
#if CONFIG_BF20A6_SUPPORT
#include "../sensor/bf20a6.h"
#endif
#if CONFIG_SC101IOT_SUPPORT
#include "../sensor/sc101iot.h"
#endif
#if CONFIG_SC030IOT_SUPPORT
#include "../sensor/sc030iot.h"
#endif
#if CONFIG_SC031GS_SUPPORT
#include "../sensor/sc031gs.h"
#endif
...
// เพิ่มบรรทัดนี้เพื่อนำเข้าค่าดีฟอลต์จาก sccb_idf4.c (ประกาศเป็น global const อยู่แล้ว)
extern const int SCCB_I2C_PORT_DEFAULT;

#if ESP_IDF_VERSION_MAJOR < 5
    static inline int SCCB_Probe(i2c_port_t port, uint8_t addr)
    {
        i2c_cmd_handle_t cmd = i2c_cmd_link_create();
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, (addr << 1) | I2C_MASTER_WRITE, true); // เช็ค ACK
        i2c_master_stop(cmd);
        esp_err_t r = i2c_master_cmd_begin(port, cmd, pdMS_TO_TICKS(200));
        i2c_cmd_link_delete(cmd);
        return r; // ESP_OK=ACK, ESP_FAIL=NACK, ESP_ERR_TIMEOUT ฯลฯ
    }
...
static ovcam_err_t camera_probe(const camera_config_t *config, camera_model_t *out_camera_model)
{
    
    ovcam_err_t ret = OVCAM_OK;

    *out_camera_model = CAMERA_NONE;
    if (s_state != NULL) {
        // return ESP_ERR_INVALID_STATE;
        return OVCAM_E_NOENT;
    }

    s_state = (camera_state_t *) calloc(1, sizeof(camera_state_t));
    if (!s_state) {
        // return ESP_ERR_NO_MEM;
        SCCB_LOGE(TAG,"ERR_NO_MEM");
        return OVCAM_E_NOMEM;
    }


    if (config->pin_xclk >= 0) {
        SCCB_LOGD(TAG, "Enabling XCLK output");
        CAMERA_ENABLE_OUT_CLOCK(config);
    }
    vTaskDelay(pdMS_TO_TICKS(50));

    bool used_init = false;
    if (config->pin_sccb_sda != -1) {
        SCCB_LOGD(TAG, "Initializing SCCB");
        ret = SCCB_Init(config->pin_sccb_sda, config->pin_sccb_scl);
        used_init = true;
        SCCB_LOGI(TAG, "SCCB cfg: SDA=%d SCL=%d port=%d mode=SCCB_Init",
                  config->pin_sccb_sda, config->pin_sccb_scl, (int)config->sccb_i2c_port);
    } else {
        SCCB_LOGD(TAG, "Using existing I2C port");
        ret = SCCB_Use_Port(config->sccb_i2c_port);
        SCCB_LOGI(TAG, "SCCB cfg: SDA=? SCL=? port=%d mode=SCCB_Use_Port",
                  (int)config->sccb_i2c_port);
    }
    if (ret != OVCAM_OK) { SCCB_LOGE(TAG, "sccb init err"); goto fail; }else{SCCB_LOGD(TAG,"I2C Initialed");}

    if (config->pin_pwdn >= 0) {
        gpio_config_t g = {0};
        g.pin_bit_mask = 1ULL << config->pin_pwdn;
    ...
    if (config->pin_reset >= 0) {
        gpio_config_t g = {0};
        g.pin_bit_mask = 1ULL << config->pin_reset;
        g.mode = GPIO_MODE_OUTPUT;
        gpio_config(&g);
    ...
    // // เลือกพอร์ตให้ตรงกับเส้นทางที่ใช้
    i2c_port_t probe_port = used_init
                            ? (i2c_port_t)SCCB_I2C_PORT_DEFAULT   // เข้าทาง SCCB_Init
                            : (i2c_port_t)config->sccb_i2c_port;  // เข้าทาง SCCB_Use_Port
...
//     for (uint8_t addr = 0; addr < 128 && *out_camera_model == CAMERA_NONE; ++addr) {

//     // ข้ามช่วง reserved เพื่อลดเวลาสแกน (ยกเว้น 0x30 ที่เราสนใจเป็นพิเศษ)
//     if ((addr < 0x08 || addr > 0x77) && addr != 0x30) {
//         continue;
//     }

//         for (int attempt = 1; attempt <= try; ++attempt) {

//         esp_err_t ret2 = SCCB_Probe(probe_port, addr);
//         if (ret2 != ESP_OK) {
//             SCCB_LOGD(TAG, "Probe try addr=0x%02X attempt %d/%d -> %s(%d)",
//                       addr, attempt, try, esp_err_to_name(ret2), (int)ret2);
//             vTaskDelay(pdMS_TO_TICKS(10));
//             continue; // ลองใหม่ (ใน addr เดิม)
//         }

//         // ============ ACK แล้ว ============
//         SCCB_LOGI(TAG, "ACK @0x%02X (attempt %d/%d)", addr, attempt, try);

//         s_state->sensor.slv_addr      = addr;
...
//             if (g_sensors[i].detect(addr, id)) {
//                 SCCB_LOGI(TAG, "Camera PID=0x%02x VER=0x%02x MIDL=0x%02x MIDH=0x%02x",
//                           id->PID, id->VER, id->MIDL, id->MIDH);

//                 camera_sensor_info_t *info = esp_camera_sensor_get_info(id);
//                 if (info) {
//                     *out_camera_model = info->model;
//                     SCCB_LOGI(TAG, "Detected %s camera at 0x%02X", info->name, addr);
//                     g_sensors[i].init(&s_state->sensor);
//                     detected = true;
//                 }
//                 break; // จบลูป detect ไม่ว่าพบหรือไม่
//                 }
//             }

//         if (detected) {
//             found = true;
//             SCCB_LOGD(TAG,"detected success");
//         } else {
//             // ACK แล้วแต่ detect ไม่ผ่าน → ไป addr ถัดไป
//             SCCB_LOGW(TAG, "ACK @0x%02X แต่ไม่รู้จักรุ่น/ไม่รองรับ → ข้ามไป addr ถัดไป", addr);
//             break;
//         }
//     } // end retry-per-addr
// } // end scan all addrs
...
uint8_t slv_addr = 0;
for (int model_id = 0; *out_camera_model == CAMERA_NONE && model_id < CAMERA_MODEL_MAX; ++model_id) {
    slv_addr = camera_sensor[model_id].sccb_addr;

    // โปรบเฉพาะ addr จากตาราง
    esp_err_t r = SCCB_Probe(probe_port, slv_addr);
    if (r != ESP_OK) {
        SCCB_LOGD(TAG, "Probe try addr=0x%02X", slv_addr);
        continue;
    }

    // เจออุปกรณ์ที่ addr นี้แล้ว → อ่าน PID/VER เพื่อยืนยันรุ่นจริง
    ...
    for (size_t i = 0; i < sizeof(g_sensors)/sizeof(g_sensors[0]); ++i) {
        if (g_sensors[i].detect(slv_addr, id)) {
            camera_sensor_info_t *info = esp_camera_sensor_get_info(id);
            if (info) {
                *out_camera_model = info->model;             // เซ็ตโมเดลที่ตรวจพบ
                SCCB_LOGI(TAG, "Detected %s camera at 0x%02X", info->name, slv_addr);
                g_sensors[i].init(&s_state->sensor);         // init ฟังก์ชันของรุ่นนั้น
        ...
        if (found && *out_camera_model != CAMERA_NONE) {
            SCCB_LOGD(TAG,"detected success");
        } else {
            // ACK แล้วแต่ detect ไม่ผ่าน → ไป addr ถัดไป
        SCCB_LOGE(TAG, "Detected camera not supported.");
        ret = ESP_ERR_NOT_SUPPORTED;
        goto fail;
        }

        SCCB_LOGD(TAG, "Doing SW reset of sensor");
...
static pixformat_t get_output_data_format(camera_conv_mode_t conv_mode)
{
    ...
    SCCB_LOGD(TAG, "Convert to %d format enabled", format);
    return format;
}
#endif

ovcam_err_t esp_camera_init(const camera_config_t *config)
{
    ovcam_err_t err;
    ...
    if (err != OVCAM_OK) {
        ...
        SCCB_LOGE(TAG, "Camera init failed with error 0x%x", err);
        return err;
    }
    ...
    if (err != OVCAM_OK) {
        ...
        SCCB_LOGE(TAG, "Camera probe failed with error 0x%x(%s)", err, esp_err_to_name(err));
        goto fail;
    }

    SCCB_LOGI(TAG, "raw config->frame_size=%d", (int)config->frame_size);

    framesize_t frame_size = (framesize_t) config->frame_size;
    SCCB_LOGI(TAG, "config->frame_size enum=%d -> %dx%d",
          (int)frame_size,
          resolution[frame_size].width,
          resolution[frame_size].height);

    for(int i = 0; i< (int)FRAMESIZE_INVALID;i++){
        SCCB_LOGD(TAG,"[%2d] -> %ux%u (aspect=%d)",i,resolution[i].width,resolution[i].height,(int)resolution[i].aspect_ratio);
    }
    ...
    if (PIXFORMAT_JPEG == pix_format && (!camera_sensor[camera_model].support_jpeg)) {
        ...
        SCCB_LOGE(TAG, "JPEG format is not supported on this sensor");
    ...
    if (frame_size > camera_sensor[camera_model].max_size) {
        ...
        SCCB_LOGW(TAG, "The frame size exceeds the maximum for this sensor, it will be forced to the maximum possible value");
    ...
    if (err != OVCAM_OK) {
        ...
        SCCB_LOGE(TAG, "Camera config failed with error 0x%x", err);
        goto fail;
    }
    ...
    SCCB_LOGD(TAG, "Setting frame size to %dx%d", resolution[frame_size].width, resolution[frame_size].height);
    if (s_state->sensor.set_framesize(&s_state->sensor, frame_size) != 0) {
        // ESP_LOGE(TAG, "Failed to set frame size");
        SCCB_LOGE(TAG, "Failed to set frame size");
...
ovcam_err_t esp_camera_deinit()
{
    ovcam_err_t ret = cam_deinit();
    CAMERA_DISABLE_OUT_CLOCK();
    if (s_state) {
        SCCB_Deinit();
...
ovcam_err_t esp_camera_save_to_nvs(const char *key)
{
...
ovcam_err_t esp_camera_load_from_nvs(const char *key)
{
#if ESP_IDF_VERSION_MAJOR > 3
    nvs_handle_t handle;
#else
    ...
    if (ret == ESP_OK) {
        ...
        SCCB_LOGW(TAG, "Error (%d) opening nvs key \"%s\"", ret, key);
        return ERR_MAP_ESP(ret);
    }
}
...
ovcam_err_t esp_camera_reconfigure(const camera_config_t *config)
{
    if (!config) {
        return OVCAM_E_INVAL;
    }
    if (s_state) {
        ovcam_err_t err = esp_camera_deinit();
        if (err != OVCAM_OK) {
            return err;
        }
    }
...
ovcam_err_t esp_camera_set_psram_mode(bool enable)
{
