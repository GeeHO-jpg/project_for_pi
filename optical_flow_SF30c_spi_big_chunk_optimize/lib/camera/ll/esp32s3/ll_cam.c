...
ovcam_err_t ll_cam_deinit(cam_obj_t *cam)
{
...
static ovcam_err_t ll_cam_dma_init(cam_obj_t *cam)
{
    ...
    if (ret != ESP_OK) {
        ...
        SCCB_LOGE(TAG, "Can't find available GDMA channel");
        return OVCAM_E_FAIL;
    }
    int chan_id = -1;
    ...
    if (ret != ESP_OK) {
        ...
        SCCB_LOGE(TAG, "Can't get GDMA channel number");
        return OVCAM_E_FAIL;
    }
    cam->dma_num = chan_id;
...
static ovcam_err_t ll_cam_converter_config(cam_obj_t *cam, const camera_config_t *config)
{
    ...
    case YUV422_TO_YUV420:
        if (config->pixel_format != PIXFORMAT_YUV422) {
            ...
            SCCB_LOGI(TAG, "YUV422 to YUV420 mode");
    ...
    case YUV422_TO_RGB565:
        if (config->pixel_format != PIXFORMAT_YUV422) {
            ...
            SCCB_LOGI(TAG, "YUV422 to RGB565 mode");
...
ovcam_err_t ll_cam_config(cam_obj_t *cam, const camera_config_t *config)
{
    ovcam_err_t ret = OVCAM_OK;
...
ovcam_err_t ll_cam_set_pin(cam_obj_t *cam, const camera_config_t *config)
{
...
ovcam_err_t ll_cam_init_isr(cam_obj_t *cam)
{
    ...
    if (ret != ESP_OK) {
        ...
        SCCB_LOGE(TAG, "DMA interrupt allocation of camera failed");
	...
	if (ret != ESP_OK) {
        ...
        SCCB_LOGE(TAG, "LCD_CAM interrupt allocation of camera failed");
		return ERR_MAP_ESP(ret);
	}
    return OVCAM_OK;
}
...
static bool ll_cam_calc_rgb_dma(cam_obj_t *cam){
    ...
    SCCB_LOGI(TAG, "node_size: %4u, nodes_per_line: %u, lines_per_node: %u",
            (unsigned) (node_size * cam->dma_bytes_per_item), (unsigned) nodes_per_line, (unsigned) lines_per_node);
    ...
    if (line_width > dma_half_buffer_max) {
        ...
        SCCB_LOGE(TAG, "Resolution too high");
        return 0;
    }
    ...
    SCCB_LOGI(TAG, "dma_half_buffer_min: %5u, dma_half_buffer: %5u, lines_per_half_buffer: %2u, dma_buffer_size: %5u",
            (unsigned) (dma_half_buffer_min * cam->dma_bytes_per_item), (unsigned) (dma_half_buffer * cam->dma_bytes_per_item),
            (unsigned) lines_per_half_buffer, (unsigned) (dma_buffer_size * cam->dma_bytes_per_item));
...
ovcam_err_t ll_cam_set_sample_mode(cam_obj_t *cam, pixformat_t pix_format, uint32_t xclk_freq_hz, uint16_t sensor_pid)
{
    if (pix_format == PIXFORMAT_GRAYSCALE) {
        ...
        // ESP_LOGE(TAG, "Requested format is not supported");
         SCCB_LOGE(TAG, "Requested format is not supported");
        return OVCAM_E_NOT_SUP;
    }
    return OVCAM_OK;
}
