...
bool TFMPlus::getData( int16_t &dist, int16_t &flux, int16_t &temp)
{
    ...
    if( status != TFMP_READY) return false;
        ...
        last_measure_time_us_ = (uint64_t)esp_timer_get_time();
        return true;
    }
...
void TFMPlus::printStatus()
{
    ...
    else if (status == TFMP_SERIAL)     printf("SERIAL");
    else if (status == TFMP_HEADER)     printf("HEADER");
    else if (status == TFMP_CHECKSUM)   printf("CHECKSUM");
    else if (status == TFMP_TIMEOUT)    printf("TIMEOUT");
    else if (status == TFMP_PASS)       printf("PASS");
    else if (status == TFMP_FAIL)       printf("FAIL");
    else if (status == TFMP_I2CREAD)    printf("I2C-READ");
    else if (status == TFMP_I2CWRITE)   printf("I2C-WRITE");
    else if (status == TFMP_I2CLENGTH)  printf("I2C-LENGTH");
    else if (status == TFMP_WEAK)       printf("Signal weak");
    else if (status == TFMP_STRONG)     printf("Signal saturation");
    else if (status == TFMP_FLOOD)      printf("Ambient light saturation");
    else                                printf("OTHER");
...
void TFMPlus::printFrame()
{
    ...
    {
        // %02X = พิมพ์เป็นเลขฐาน 16 กว้าง 2 หลัก เติม 0 ข้างหน้าให้ครบ
...
void TFMPlus::printReply()
{
    ...
    {
        printf(" %02X", (unsigned int)reply[i]);
    }
    printf("\n");
}