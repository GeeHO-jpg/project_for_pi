...
// CMD_DATA payload (5 chunks @ DATA_CHUNK_SIZE=4, chunk สุดท้าย 3 byte)
static const uint8_t k_payload[SPI_COMM_DATA_PAYLOAD_SIZE] = {
    0xFF, 0xFE, 0xFD, 0xFC, 0xFB, 0xFA, 0xF9, 0xF8,
    0xF7, 0xF6, 0xF5, 0xF4, 0xF3, 0xF2, 0xF1, 0xF0,
    0xEF, 0xEE, 0xED
};

// ขนาด/จำนวน chunk ของ k_payload (CMD_DATA) ที่ฝั่งนี้จะส่งจริง
    ...
    while (true) {
        ...
        if (pkt) {
            if (pkt->header->cmd == CMD_PROTO_INFO) {
                ...
                uint8_t chunk_index = (pkt->header->payload_size >= 1) ? pkt->payload[0] : 0;

                resp_buf[0] = chunk_index;
                get_chunk(&resp_buf[1], k_payload, SPI_COMM_DATA_PAYLOAD_SIZE, DATA_CHUNK_SIZE, chunk_index);
